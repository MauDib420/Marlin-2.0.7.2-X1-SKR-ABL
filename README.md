...
This is a tested build based on my previous configuration (2.0.7). I've enabled most of the debugging features (M48 etc.) so you're able to work your way to ideal settings. Threshhold for sensorless homing is set to working values - yours might differ from that. Review and set with [M914](https://marlinfw.org/docs/gcode/M914.html).

Linear Advance is activated but disabled by setting the value to zero.

Review the Firmware info with M115 and my settings with M503. PID-Tuning for both hotend and bed is a must so is calibrating x, y, z and e steps! Stepper driver current is set to 0.950. The BLTouch is connected using the LED pins on the extruder:
...
Don't forget to note down your personal printer settings (steps, etc.). Reset to factory defaults after flash and save the settings (M502 + M500).
